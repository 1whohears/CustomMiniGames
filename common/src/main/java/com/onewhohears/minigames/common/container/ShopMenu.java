...
public class ShopMenu extends AbstractContainerMenu {
	...
	public static void openScreen(ServerPlayer player, GameShop shop) {
        MenuRegistry.openExtendedMenu(player, new SimpleMenuProvider((windowId, playerInv, p) ->
	...
	public ShopMenu(int windowId, Inventory playerInv, FriendlyByteBuf buffer) {
		this(windowId, playerInv, getShop(buffer));
	}

    private static GameShop getShop(FriendlyByteBuf buffer) {
        String shop_name = buffer.readUtf();
        return MiniGameShopsManager.get().get(shop_name);
    }
